...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (profile) {
      ...
      contextInfo += `Academic Level: ${
        profile.academic_level || 'Not provided'
      }\n`;
      contextInfo += `Interests: ${
        profile.interests?.join(', ') || 'Not provided'
      }\n`;
      contextInfo += `Skills: ${
        profile.skills?.join(', ') || 'Not provided'
      }\n`;
      if (profile.academic_scores) {
        contextInfo += `Academic Scores: ${JSON.stringify(
          profile.academic_scores
        )}\n`;
      }
    }
...
- If anyone asks for information outside of academic guidance, politely decline and also decline Hello, to provide medical, legal, or financial advice.
